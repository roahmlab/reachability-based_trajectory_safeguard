classdef MATLABEnvironment < AbstractEnv
% MATLABENVIRONMENT
        ...
        function [experiences,episodeInfo,simInfo,workerID] = simLoop(env,policy,opts,simCount,usePCT)
            ...
            try
                ...
                for stepCount = 1:opts.MaxSteps
                    ...
                    % step the policy
                    action = step(policy,observation,reward,isdone,loggedSignals);

                    if abs(isdone - 1) < 0.1 || abs(isdone - 3) < 0.1 || abs(isdone - 4) < 0.1 || abs(isdone - 5) < 0.1
                        break;
                    end
                end
            catch ex
