classdef rlSACAgent < rl.agent.AbstractAgent
    % rlSACAgent: Implements a soft actor critic with
    ...
    methods
        function this = rlSACAgent(Actor, Critic1, varargin)
            % Constructor
        ...
        function Action = getActionWithExploration(this, Observation)
            ...
            Action = scaleActions(this, Action);
        end
        function ScaledActions = scaleActions(this, Actions)
            % REVISIT: does not support multi action channel
            % REVISIT: handle Inf action bound
            % should this be a public method?
            SquashedActions = tanh(Actions{1});
            ScaledActions = SquashedActions * this.ActionScale + this.ActionShift;
        end
        ...
        function this = setActor(this, Actor)
            ...
            % set SAC loss
            Actor = setLoss(Actor, "rl.loss.sac");
    ...
    methods (Access = protected)
        ...
        function Action = getActionImpl(this, Observation)
            ...
            NumActions = prod(this.ActionInfo.Dimension);
            Action = ProbabilityParams(1:NumActions, :);
            Action = {Action};
            Action = scaleActions(this, Action);
        end
    ...
    methods (Access = private)
        ...
        function this = setActionScale(this)
            % REVISIT: use numeric spec method instead. Should this be on
            % sampling spec?
        ...
        function varargout = trainCriticWithBatch(this, miniBatch)
            ...
            AdjustedNextActions = scaleActions(this, NextActions);
            AdjustedNextActions = {AdjustedNextActions};
            Densities = evaluate(this.Actor.SamplingStrategy, ProbabilityParams{1}, NextActions{1});
            ...
            PDFCorrection = sum(log(rl.internal.dataTransformation.boundAwayFromZero((1 - tanh(NextActions{1}).^2) .* this.ActionScale)), 1);
        ...
        function varargout = trainActorWithBatch(this, miniBatch)
            ...
            LossVariable.ActionScale = this.ActionScale;
            LossVariable.ActionShift = this.ActionShift;
        ...
        function trainEntropyWeight(this, miniBatch)
            if this.LearnableEntropy
                ...
                Densities = evaluate(this.Actor.SamplingStrategy, ProbabilityParams{1}, Actions{1});
                Densities = prod(Densities, 1);
                LogDensities = log(rl.internal.dataTransformation.boundAwayFromZero(Densities));
                PDFCorrection = sum(log(rl.internal.dataTransformation.boundAwayFromZero((1 - tanh(Actions{1}).^2) .* this.ActionScale)), 1);
                LogDensities = LogDensities - PDFCorrection;
