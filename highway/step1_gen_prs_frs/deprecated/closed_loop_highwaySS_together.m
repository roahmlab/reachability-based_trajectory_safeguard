function dz = closed_loop_highwaySS_together(tdummy,z,udummy)
...
% options.x0 = [0;0;0;kvc;0;    kvc;   kyc];
            ...
            vx_nom =z(6)*(-1/(1+exp(-5*(t-1.3+0.5)))+1);%(1-0.05*(t+0.2)^3); % this is the output from network,network output discrete, converted to num by planner
            ...
            ua = 1/2*(vx_nom-vx);
            delta_cmd = 0.015*(y_nom-y)-0.3*psi-0.1*delta;
