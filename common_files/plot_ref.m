function plot_ref(AH,ref, proposed_ref)
Z_prime = ref;
if ~isempty(proposed_ref)
    ...
    if check_if_plot_is_available(AH,'proposed_trajectory')
        ...
        trajectory_data = plot(Z(1,:),Z(2,:),'k-','LineWidth',3);
        traj2           =plot(Z(1,:),Z(2,:),'Color','y','LineWidth',3,'LineStyle','--');
...
if ~isempty(ref)
    ...
    if check_if_plot_is_available(AH,'trajectory')
        ...
        trajectory_data = plot(Z(1,:),Z(2,:),'k-',...
            'LineWidth',3) ;
